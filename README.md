...
## Задачи, для решения которых был создан сервис
Единый сервис позволяет решать различные задачи для отдельных систем:
1. Определение семантической схожести текстов.
- быстрая фильтрация результатов поиска по ключевым словам в DeepResearch pipeline (без векторной БД).
2. Определение схожести перечня текстов с эталоном.
- Используется для проверки гипотез, сгенерированных LLM (техника HyDE - Hypothetical Document Embeddings) в RAG-системе.
3. Векторизация текстового фрагмента (чанка) для сохранения в БД ChromaDB в RAG-системе с векторной БД.
